...
Snapshots bind exact base/head SHAs and provenance. Merge reconstructions are explicitly marked as merge snapshots, never as opening-time snapshots. Multi-commit rebases use the original PR head and merge base when verifiable. Renames/deletions are preserved; unsupported binary content prevents classification. New snapshots also capture available PR title/description and their collection provenance; blind review packets exclude those fields. Metadata is refreshed when reusing the immutable Git snapshot cache.
